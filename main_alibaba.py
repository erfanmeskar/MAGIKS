...
if __name__ == "__main__":
    ...
    N, E, num_save, num_sample, run_num = argv[0], argv[1], argv[2], argv[3], argv[4]
    Slist = [1 for i in range(E)]
    c_lst = [[64.0, 512.0, 2.0], [96.0, 512.0, 8.0], [96.0, 512.0, 2.0], [96.0, 384.0, 8.0]]
    c_b_lst = [50e6, 150e6, 200e6, 250e6, 300e6]
    df = pd.read_csv('./56_sampled5_event_nonzero.csv', header=0, index_col=0)
    df = df[['req_cpu_core', 'req_mem_GB', 'req_gpu', 'req_bit_per_sec']]
    df.drop(df[df.req_cpu_core == 0].index, inplace=True)
    df.drop(df[df.req_mem_GB == 0].index, inplace=True)
    df.drop(df[df.req_gpu == 0].index, inplace=True)
    df.drop(df[df.req_bit_per_sec == 0].index, inplace=True)
    ...
    for save_iter in range(num_save):
        ...
        for sample_iter in range(num_sample):
            ...
            c = np.array([random.choice(c_lst)])
            c_b = random.choice(c_b_lst)
            d = df.sample(n=N, replace=False).to_numpy()
            d[:, 3] /= 3.5
            clusters = []
            aps = []
            for i in range(E):
            ...
            env_ks_rand, env_ks_max, env_mnw_eq, env_ks_eq, env_eq = (HomoEnv(deepcopy(agents), deepcopy(aps)) for i in range(5))
            env_pool_ks, env_pool_norm_utilitarian = (VirtEnv(deepcopy(agents), deepcopy(aps)) for i in range(2))
            ...
            env_pool_norm_utilitarian.initialize(how='given', opt='ks', init_loc=[0 for i in range(N)])
        ...
        name = f"./simulation_result/{N}_{E}_{save_iter}_{run_num}_alibaba.pickle"
        save_dic = {'res_util': {'ks_rand': res_util_ks_rand,
                                 'ks_max': res_util_ks_max,
                                 'ks_eq': res_util_ks_eq,
                                 'mnw_eq': res_util_mnw_eq,
                                 'ks_pool': res_util_virtual_ks,
                                 'opt_pool': res_util_virtual_norm_utilitarian,
                                 'eq': res_util_eq},
                    ...
                    'sum_util': {'ks_rand': sum_util_ks_rand,
                                 ...
                                 'ks_pool': sum_util_virtual_ks,
                                 'opt_pool': sum_util_virtual_norm_utilitarian,
                                 'eq': sum_util_eq},
                    'sum_norm_util': {'ks_rand': sum_norm_util_ks_rand,
                                      ...
                                      'ks_pool': sum_norm_util_virtual_ks,
                                      'opt_pool': sum_norm_util_virtual_norm_utilitarian,
                                      'eq': sum_norm_util_eq}}
